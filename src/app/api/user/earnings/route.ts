...
export async function GET(request: NextRequest) {
  ...
  const totalEarnings = purchases?.reduce((sum, p) => sum + Number(p.seller_amount), 0) || 0;
  ...
  const pendingPayout = pendingPayouts?.reduce((sum, p) => sum + Number(p.amount), 0) || 0;
  ...
  for (let i = daysBack - 1; i >= 0; i--) {
    ...
    const dayEarnings = purchases
      ...
      .reduce((sum, p) => sum + Number(p.seller_amount), 0) || 0;
